...
class SpyNet(nn.Module):
    ...
    def __init__(self, load_path=None):
        ...
        if load_path:
            self.load_state_dict(
                torch.load(load_path, map_location=lambda storage, loc: storage)["params"]
            )
    ...
    def process(self, ref, supp):
        ...
        for level in range(5):
            ref.insert(
                0, F.avg_pool2d(input=ref[0], kernel_size=2, stride=2, count_include_pad=False)
            )
            supp.insert(
                0, F.avg_pool2d(input=supp[0], kernel_size=2, stride=2, count_include_pad=False)
            )
        ...
        for level in range(len(ref)):
            upsampled_flow = (
                F.interpolate(input=flow, scale_factor=2, mode="bilinear", align_corners=True) * 2.0
            )
    ...
    def forward(self, ref, supp):
        ...
        flow = F.interpolate(
            input=self.process(ref, supp), size=(h, w), mode="bilinear", align_corners=False
        )
